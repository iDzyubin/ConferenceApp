...
const UserReports = props => {
  ...
  const handleSubmit = async e => {
    let formObj = document.getElementById('edit-report-form');
    ...
    <div>
      {props.reports.length > 0 ? (
        <Table>
          ...
          <tbody>
            {props.reports.map(r => (
              <TableRow key={r.id} onClick={() => toggleModalWindow(r)}>
      ...
      <ModalWindow style={getModalState()}>
        <ModalContent>
          ...
          <Form id="edit-report-form" onSubmit={e => e.preventDefault()}>
            <FormGroup>
              <InputText
